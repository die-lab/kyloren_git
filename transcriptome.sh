...
SRA_entries="SRR281065,SRR281063,SRR281061,SRR280913,SRR280911,SRR280842"
SRA_entries=`echo $SRA_entries | sed 's/,/ /g'`
...
if [ $layout=="paired" ]
	...
	then for fastq in *_1.fastq
		do trimmomatic PE threads $num_threads $fastq ${fastq%_1.fastq}_2.fastq -baseout ${fastq%_1.fastq}_trimmed ILLUMINACLIP:$adapter_file:2:30:10 AVGQUAL:20 LEADING:3 TRAILING:3 SLIDINGWINDOW:25:33 MINLEN:$minlen
	fastqc $fastq
	done
...
#sh stats_on_analyses.sh fastq paired 
	...
	kraken2 --db /home/dbs/kraken_dbs --threads $num_threads --output kraken2.out --paired --use-names --report kraken2.report --classified-out classified_out# classified_out# --unclassified-out unclassified_out# unclassified_out# trimmed_fixed_*P
...
java -jar /opt/Trimmomatic-0.38/trimmomatic-0.38.jar SE -threads $num_threads $fastq ${fastq%.fastq}.trim.fastq ILLUMINACLIP:$adapter_file:2:30:10 AVGQUAL:20 LEADING:3 TRAILING:3 SLIDINGWINDOW:25:33 
	...
	for fastq in *.trim.fixed.fastq
		do kraken2 --db /home/dbs/kraken_dbs  --threads $num_threads --output kraken2.out --use-names --report kraken2.report --classified-out ${fastq%.fixed.fastq}.cont.fastq --unclassified-out ${fastq%.fixed.fastq}.kraken2.fastq $fastq
	        done
